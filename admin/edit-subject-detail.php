...
if (strlen($_SESSION['sturecmsaid'] == 0)) {
  ...
  if (isset($_POST['submit'])) {
    ...
    $sql = "update tblsubjects set SubjectName=:sname,Year=:year,Semester=:semester,GroupName=:groupName where s_id=:eid";
    $query = $dbh->prepare($sql);
    $query->bindParam(':sname', $SubjectName, PDO::PARAM_STR);
  ...
  <head>
    ...
    <title>MIET BR ADMIN || Manage Subjects</title>
    <!-- plugins:css -->
          ...
          <div class="content-wrapper">
            ...
            <div class="row">
              ...
              <div class="col-12 grid-margin stretch-card">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title" style="text-align: center;">Manage Subjects</h4>

                    <form class="forms-sample" method="post">
                      <?php
                      $eid = $_GET['editid'];
                      $sql = "SELECT * from  tblsubjects where s_id=$eid";
                      $query = $dbh->prepare($sql);
                      $query->execute();
                      ...
                      if ($query->rowCount() > 0) {
                        foreach ($results as $row) { ?>
                          <div class="form-group">
                            ...
                            <input type="text" name="sname" value="<?php echo htmlentities($row->SubjectName); ?>"
                              class="form-control" required='true'>
                          </div>
                          <div class="form-group">
                            <label for="exampleInputEmail3">Year</label>
                            <select name="year" class="form-control" required='true'>
                              <option value="<?php echo htmlentities($row->Year); ?>"><?php echo htmlentities($row->Year); ?>
                              </option>
                    ...
                    <button type="submit" class="btn btn-primary mr-2" name="submit">Update Subject</button>
